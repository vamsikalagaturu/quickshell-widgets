...
// NOTE: duplicated from connectivity/Theme.qml. Quickshell runs the launcher
// as its own config dir (separate process, separate qmldir), and a singleton
// must be registered in the qmldir of the module importing it -- sharing one
// copy would mean a `common/` module plus a new symlink in ~/.config/quickshell
// and touching the import line of every file in the network panel. Two small
// files is the cheaper trade. Keep `scale` in sync across both.
